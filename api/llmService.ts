...
export interface LLMProvider {
  name: string;
  isAvailable(): boolean;
  generate(messages: LLMMessage[], options?: LLMRequestOptions): Promise<string>;
}
...
export class GeminiProvider implements LLMProvider {
  ...
  async generate(messages: LLMMessage[], options?: LLMRequestOptions): Promise<string> {
    const key = process.env.GEMINI_API_KEY || process.env.GEMINI_API;
    if (!key) {
...
export class LLMService {
  ...
  private isRateLimitOrExhaustionError(error: any): boolean {
